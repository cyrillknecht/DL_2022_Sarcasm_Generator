...
# (NOTE: Per "epoch", a total of DATASET_SAMPLES_PER_EPOCH + GENERATED_SAMPLES_PER_EPOCH will be generated)
...
prompts = pd.read_json(NON_SARCASM_DATASET_PATH,
                       lines=True)  # train_data[train_data["label"] == "NOT_SARCASM"][['context']]
...
for epoch in range(EPOCHS):
    ...
    with open(TRAIN_SET_PATH, 'w') as f:
        ...
        while nGenerated < GENERATED_SAMPLES_PER_EPOCH and generatorTries < MAX_GENERATOR_TRIES:
            ...
            generated_tweets = gpt2.generate(sess, run_name='run_self_augmentation', seed=42, return_as_list=True,
                                             length=128,
                                             prefix=prefix, nsamples=GENERATION_BATCH_SIZE,
                                             batch_size=GENERATION_BATCH_SIZE, truncate="<|endoftext|>")
        ...
        print(
            f"Finished sample generation. {nGenerated} generated samples were classified as 'sarcastic' (out of {generatorTries} generated samples)")
...
with open(RESULT_PATH, 'w') as f:
    ...
    for i, row in prompts.head(
            GENERATE).iterrows():  # Note: i is the _original_ row number, which means i is not just a "counter variable"
        prefix = "<sc> " + row['context'] + " <ec> <sr> "
        generated_tweet = \
            gpt2.generate(sess, run_name='run_self_augmentation', seed=42, return_as_list=True, length=128, prefix=prefix,
                          nsamples=1, batch_size=1, truncate="<|endoftext|>")[0]
        generated_tweet = generated_tweet.replace("\r", " ").replace("\n", " ").replace(",", "").replace(";",
                                                                                                         "").strip()
        f.write(generated_tweet + "\n")
        if counter % 5 == 0:

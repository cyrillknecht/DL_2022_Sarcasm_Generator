...
for epoch in range(EPOCHS):
    ...
    print(
        f"Running {current_number_of_samples} iterations on {current_number_of_samples} samples..."
    )
    ...
    with open(TRAIN_SET_PATH, "w") as f:
        ...
        while (
            nGenerated < int(GENERATED_SAMPLES_PER_EPOCH)
            and generatorTries < MAX_GENERATOR_TRIES
        ):

            # Kinda stupid: we can only supply a single prefix even when generating multiple samples...
            prefix = (
                "<sc> "
                + prompts.iloc[promptIndex % NUM_NON_SARCASTIC_TWEETS]["context"]
                + " <ec> <sr> "
            )
            generated_tweets = gpt2.generate(
                sess,
                run_name="run_self_augmentation",
                return_as_list=True,
                length=128,
                prefix=prefix,
                nsamples=GENERATION_BATCH_SIZE,
